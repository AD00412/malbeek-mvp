/* ============================================================
   ملبّيك · مخطط الباص (قابل للضبط) + سياسات التخصيص
   - صفوف بتشكيلة 2 + ممر + 2  (٤ مقاعد لكل صف)
   - صف خلفي متراص (افتراضيا ٥ مقاعد)
   - الترقيم من اليمين لليسار، يبدأ من مقدمة الباص
   - الأبواب دائما يمينا · السائق ومساعده أعلى اليسار (بصريا فقط)
   ============================================================ */
...
/** إجمالي المقاعد لتخطيط معين */
export function seatCount(rows = DEFAULT_ROWS, back = DEFAULT_BACK) {
  return rows * 4 + back
}

/**
 * توليد كل المقاعد بمواقعها (row, col) لتخطيط قابل للضبط.
 * الأعمدة يمين←يسار: 0(نافذة يمين) 1(ممر يمين) [ممر] 3(ممر يسار) 4(نافذة يسار)
 */
...
/* هل المعتمر مسموح بهذا المقعد؟
   - any:    متاح للجميع
   - family: منطقة العوائل — للعائلات فقط (ذكورا أو إناثا ضمن عائلة)
   - male/female: لجنس محدد، وتقبل العائلة من نفس الجنس أيضا */
export function isAllowed(seat, policy, gender, isFamily) {
  const a = allowedFor(seat, policy)
  if (a === 'any') return true
  if (a === 'family') return !!isFamily      // منطقة العوائل محصورة بالعائلات
