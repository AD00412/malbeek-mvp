-- ============================================================
-- ملبّيك · إعادةُ هيكلة الإدارة — الخطوة ٢/٢
-- ============================================================
-- شغّل هذا بعد STEP1 (الذي أضاف دور 'support').
-- يَحوي: الأعمدة + جدول audit + ٨ RPCs. idempotent بالكامل.
-- ============================================================

-- ─── أعمدةُ subscribers الجديدة ───
...
comment on column public.subscribers.trial_extended_until is 'تَمديدُ تَجربةٍ يَدويٌّ من الإدارة.';
...
create or replace function public._log_admin_action(
  p_action text, p_target_type text, p_target_id uuid, p_target_label text, p_details jsonb default '{}'::jsonb
) returns void
...
begin
  ...
  values (auth.uid(), v_name, v_role, p_action, p_target_type, p_target_id, p_target_label, coalesce(p_details, '{}'::jsonb));
end $$;

-- ─── RPC: تَمديد التَّجربة ───
...
begin
  ...
  perform public._log_admin_action('extend_trial', 'subscriber', p_sub, v_org,
...
returns void
language plpgsql security definer set search_path = public, pg_catalog as $$
declare v_org text; v_prev text;
begin
  if my_role() <> 'admin'::user_role then raise exception 'admin-only' using errcode = '42501'; end if;
  update public.subscribers set suspended_at = null, suspended_reason = null where id = p_sub
    returning org_name, suspended_reason into v_org, v_prev;
  if not found then raise exception 'not-found' using errcode = '02000'; end if;
  perform public._log_admin_action('restore', 'subscriber', p_sub, v_org, jsonb_build_object('prev_reason', v_prev));
end $$;
...
begin
  ...
  update public.subscribers
    set plan = p_plan, plan_started_at = case when p_plan = 'paid' then now() else plan_started_at end
    where id = p_sub;
  perform public._log_admin_action('plan_change', 'subscriber', p_sub, v_org,
...
begin
  if my_role() <> 'admin'::user_role then raise exception 'admin-only' using errcode = '42501'; end if;
  update public.subscribers set admin_notes = nullif(trim(coalesce(p_note,'')), '') where id = p_sub returning org_name into v_org;
  if not found then raise exception 'not-found' using errcode = '02000'; end if;
...
begin
  ...
  if v_id is null then
    raise exception 'user-not-found' using errcode = '02000',
      hint = 'يَجب أن يُنشئَ المستخدمُ حسابَه أوّلًا على mulabeek.com.';
  end if;
  select role into v_old from public.profiles where id = v_id;
  update public.profiles set role = v_role where id = v_id;
  perform public._log_admin_action('staff_role_set', 'profile', v_id, p_email,
    jsonb_build_object('from', v_old::text, 'to', v_role::text));
  return v_id;
end $$;

-- ─── RPC: نَزع دور ───
create or replace function public.platform_revoke_role(p_profile uuid)
returns void
language plpgsql security definer set search_path = public, pg_catalog as $$
...
begin
  ...
  if p_profile = auth.uid() then
    raise exception 'cannot-revoke-self' using errcode = '22023', hint = 'لا تَنزع صلاحيّاتِك بنفسك.';
  end if;
  select role into v_old from public.profiles where id = p_profile;
  if v_old not in ('admin'::user_role, 'support'::user_role) then raise exception 'not-staff' using errcode = '22023'; end if;
  ...
  perform public._log_admin_action('staff_removed', 'profile', p_profile, v_email, jsonb_build_object('from', v_old::text));
end $$;

-- ─── RPC: قائمة الفريق ───
create or replace function public.platform_list_staff()
returns table(profile_id uuid, full_name text, email text, role user_role, created_at timestamptz)
language plpgsql security definer set search_path = public, pg_catalog as $$
begin
...
-- ─── تحقّق ───
select 'support role' as item, case when exists(select 1 from pg_enum e join pg_type t on t.oid=e.enumtypid where t.typname='user_role' and e.enumlabel='support') then 'OK ✓' else 'MISSING ✗' end as status
union all select 'audit table', case when to_regclass('public.platform_audit_log') is not null then 'OK ✓' else 'MISSING ✗' end
union all select 'suspend cols', case when exists(select 1 from information_schema.columns where table_name='subscribers' and column_name='suspended_at') then 'OK ✓' else 'MISSING ✗' end
union all select 'RPC count', (select count(*)::text from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('extend_subscriber_trial','suspend_subscriber','restore_subscriber','set_subscriber_plan','set_subscriber_admin_note','platform_grant_role','platform_revoke_role','platform_list_staff')) || ' / 8';
