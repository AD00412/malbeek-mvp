...
/**
 * تنظيفُ المدخل من المستخدم:
 * - يزيل CR/LF (يمنع email header injection)
 * - يزيل أحرف التحكّم (control chars) عدا التابات
 * - يحدّ الطولَ بمسافةٍ آمنة
 */
function sanitize(s: unknown, maxLen = 500): string {
  return String(s ?? '')
    // CRLF + Unicode line/paragraph separators (U+2028/U+2029) → مسافة
    .replace(/[\r\n\u2028\u2029]+/g, ' ')
    // أحرف التحكّم C0/C1 عدا التاب → تُحذف
    .replace(/[\u0000-\u0008\u000B-\u001F\u007F-\u009F]/g, '')
    .slice(0, maxLen)
    .trim()
}
...
/**
 * تحقّقٌ بزمنٍ ثابتٍ من السرّ — يمنع timing-attacks
 * (المقارنة الاعتياديّة a !== b تتوقّف عند أوّل اختلاف، فيُمكن استنتاج طول
 *  ومحتوى السرّ بقياس زمن الردّ).
 */
function timingSafeEq(a: string, b: string): boolean {
  if (a.length !== b.length) {
    // نقوم بعمليّةٍ زائفةٍ لمساواة الزمن مع حالة المساواة في الطول
    let r = 0
...
/**
 * يتحقّق أنّ مسار التخزين آمنٌ:
 * - يبدأ بـ `public/` (المجلّد المسموح للزوّار)
 * - لا يحوي `..` (path traversal)
 * - لا يحوي backslash (تغافلٌ شائعٌ على Windows-style)
 * - طولٌ معقول
 */
...
Deno.serve(async (req) => {
  ...
  // ١) تحقّقٌ من سرّ الـ webhook (مقارنةٌ بزمنٍ ثابتٍ — حمايةٌ من timing attacks)
  if (HOOK_SECRET) {
    const got = req.headers.get('x-webhook-secret') ?? ''
    if (!timingSafeEq(got, HOOK_SECRET)) return json(401, { error: 'unauthorized' })
  }
  ...
  if (!raw?.id || !raw?.body) return json(400, { error: 'no_record' })

  // تنظيفٌ دفاعيٌّ لكلّ مدخلٍ من المستخدم — حتّى لو الـ RPC نظّفها مسبقًا
  // (defense in depth): نمنع email header injection و XSS reflection.
  const rec = {
    id:          String(raw.id),
    mode:        raw.mode === 'feedback' ? 'feedback' : 'contact',
    kind:        sanitize(raw.kind, 40),
    name:        sanitize(raw.name, 120),
    email:       sanitize(raw.email, 200).toLowerCase(),
    subject:     raw.subject ? sanitize(raw.subject, 200) : null,
    body:        sanitize(raw.body, 8000),
    attachments: Array.isArray(raw.attachments) ? raw.attachments : [],
    created_at:  raw.created_at,
  }

  // تحقّقٌ إضافيٌّ من صيغة البريد — حمايةٌ مزدوجةٌ ضدّ بيانات تالفة
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(rec.email)) {
    return json(400, { error: 'invalid_email' })
  }
  if (rec.name.length < 1 || rec.body.length < 1) {
    return json(400, { error: 'empty_field' })
  }
  ...
  //    نُسقط أيّ مسارٍ غير آمنٍ (لا يبدأ بـ public/، أو يحوي ../).
  const paths: string[] = rec.attachments.filter(isSafePath).slice(0, 3)
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#f4faf6;padding:32px 16px">
    <tr><td align="center">
      ...
      <table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 4px 24px rgba(6,95,70,.08)">
        ...
        <tr><td style="padding:18px 28px 0">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;border:1px solid #e6efe9;border-radius:12px;overflow:hidden">
            ...
            <tr>
              ...
              <td style="padding:12px 16px;border-bottom:1px solid #e6efe9;font-size:14px"><a href="mailto:${encodeURIComponent(rec.email)}" style="color:#047857;font-weight:600;text-decoration:none" dir="ltr">${esc(rec.email)}</a></td>
            </tr>
            ${rec.subject ? `<tr>
        ...
        <!-- زرّ الردّ السريع (URL-encoded email لتجنّب كسر mailto: لو حوى + أو &) -->
        <tr><td style="padding:24px 28px 8px" align="center">
          <a href="mailto:${encodeURIComponent(rec.email)}?subject=${encodeURIComponent('Re: ' + subjectLine)}"
